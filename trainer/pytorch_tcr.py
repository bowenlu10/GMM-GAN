...
import math
# from trainer.utils import tensor2image
...
class TCR(nn.Module):
    def __init__(self):
        ...
        self.max_tx, self.max_ty = 0.0, 0.0  # Change as per the task
        self.min_tx, self.min_ty = -0.0, -0.0  # Change as per the task
...
class Transformation(torch.nn.Module):
    ...
    def forward(self, input, random):
        ...
        max_v = torch.tensor([[self.max_rotation_rad, self.max_translation_x, self.max_translation_y, 1]],
                             device=device)
        random_par = random * 2 * max_v - max_v
        # print('random_par: ', random_par)
        r = random_par[:, 0]
        tx = random_par[:, 1]
        ty = random_par[:, 2]
        ...
        T_trans_in = torch.cat(
            [torch.cat([T11, T12, T13], dim=2), torch.cat([T21, T22, T23], dim=2), torch.cat([TZ, TZ, TO], dim=2)],
            dim=1)
        ...
        T_in = torch.matmul(trans_mat(W_in / 2, H_in / 2, device=device),
                            torch.matmul(T_trans_in, trans_mat(-W_in / 2, -H_in / 2, device=device)))
